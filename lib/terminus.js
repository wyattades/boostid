...
exports.assertExists = async () => {
  ...
  if (!siteMap) {
    const userId = config.get('session').user_id;

    siteMap = {};

    const [ sites, orgs ] = await Promise.all([
      api(`/users/${userId}/memberships/sites?limit=500`),
      api(`/users/${userId}/memberships/organizations?limit=100`),
    ]);
    ...
    await Promise.all(orgs.map((org) => api(`/organizations/${org.id}/memberships/sites?limit=500`).then((orgSites) => {
      for (const site of orgSites) siteMap[site.site.name] = site.id;
    })));
  }

  if (!(sitename in siteMap))
    throw 'You do not have access, or it does not exist';
...
/**
 * Your site might break unless you run "clearCache" after this command
 * @async
 * @param {'git'|'sftp'} type
 */
...
exports.multidevMergeToDev = async (multidevId) => {
  ...
  assertValidMultidev(multidevId, true);
...
exports.remoteRun = async (env, ...cmd) => {
  const id = config.get('id');
  if (!id)
    throw 'Must provide config "id"';

  // cmd = Array.isArray(cmd) ? cmd : cmd.split(/\s+/);
  if (!cmd.length)
    throw 'Must provide a command';

  const sshKey = config.get('ssh.privateKey');

  const res = await run([
    'ssh', '-T', `${env}.${id}@appserver.${env}.${id}.drush.in`, '-p', '2222',
    ...(sshKey ? ['-i', sshKey] : []),
    '-o', 'StrictHostKeyChecking=no', '-o', 'AddressFamily=inet', '-o', 'BatchMode=yes',
    ...cmd,
  ], true);
