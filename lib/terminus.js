...
const api = (path, body, method) => {
  path = path.replace(/\/:siteId\//g, () => {
    ...
    if (site.length <= 11) {
      if (!(site in siteMap)) throw `You do not have access to the site: ${site}`;
      site = siteMap[site].id;
    }
  ...
  // Format error
  .catch((err) => {
    err.message = `Terminus: ${err.status} - ${typeof err.body === 'string' ? err.body.trim() : err.message}`;
    throw err;
  });
};
