...
const reportSlack = async (failed, testResultsUrl) => {
  ...
  let buildUrl = process.env.CIRCLE_BUILD_URL;
  if (!buildUrl.includes('://')) buildUrl = 'https://' + buildUrl;
  const { pathname, hostname } = require('url').parse(buildUrl);
  buildUrl = `https://${hostname}${pathname.replace(':', '')}`;
...
const uploadResults = async (testResults) => {
  ...
  //   throw 'Must provide AWS credentials';
...
module.exports = async (failed = false, testResults = null) => {
...
if (require.main === module) {
  module.exports(process.argv.includes('failed'));
}
