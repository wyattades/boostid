...
const uploadResults = async (testResults) => {
  ...
  let diffFiles = [];
  try {
    diffFiles = fs.readdirSync(TEST_RESULTS_DIR).filter((filename) => /\.(png|jpe?g)$/.test(filename));
  } catch (_) {}
  ...
  await Promise.all(diffFiles.map((filename) => (
    s3.putObject({
      Body: fs.readFileSync(`${TEST_RESULTS_DIR}/${filename}`),
      Bucket: bucket,
...
module.exports = async (failed) => {
  ...
  try {
    testResults = fs.readJSONSync(`${TEST_RESULTS_DIR}/results.json`);
  } catch (_) {
