...
const api = (path, body, method) => {
  ...
  // Format error
  .catch((err) => {
    err.message = `CircleCI: ${err.status} - ${typeof err.body === 'string' ? err.body.trim() : err.message}`;
    throw err;
  });
};
...
// exports.projectExists = async (gitUrl) => {
...
exports.createProject = async (gitUrl, envs) => {
  ...
  if (exists) {
    logger.warn('Project already exists on CircleCI. Updating environment variables');
  } else {
