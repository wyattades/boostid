...
const followCI = async ({ git, slackWebhook }) => {
  ...
  const privateKey = fs.readFileSync(config.get('ssh.privateKey'), 'utf8');
  await ci.addSSHKey(privateKey);
...
const pantheonSetup = async () => {
  ...
  logger.info(`Creating/updating "${multidev}" multidev from "dev"... (this may take a while)`);
...
const loadSSH = (keyPath) => {
  ...
  if (!fs.existsSync(privateKeyPath) || !fs.existsSync(publicKeyPath))
    return 'Public or private key does not exist';
...
You can create a new key by running "ssh-keygen -f id_rsa_boostid" and not providing a password\n\
(Make sure to add any new keys to your Pantheon dashboard)';
    ...
    return 'Failed to load specified SSH key';
  }
};
...
Please create a Pantheon site manually then run `boostid setup -s <sitename>`';
  ...
  const answers = await inquirer.prompt([{
    ...
    default: () => path.resolve(os.homedir(), '.ssh/id_rsa_pantheon'),
    validate: loadSSH,
  }, {
    type: 'input',
    name: 'slackWebhook',
