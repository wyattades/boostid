...
const followCI = async ({ git_url, slackWebhook }) => {

  log.info('Creating/updating project on CircleCI');

  ci.setGitUrl(git_url);
  config.setGlobal(`git.${ci.getOwner()}.vcsType`, ci.getVcs());
...
Please create a Pantheon site manually then run `boostid setup -s <sitename>`';
  ...
  try {
    ...
    throw `Cannot access site "${sitename}" on Pantheon. Error: ${err}`;
  }
  ...
  const { dir } = await inquirer.prompt([{
    type: 'input',
    name: 'dir',
    message: `Clone project into directory:`,
    default: sitename,
  ...
  await run(`git clone ${pantheonOrigin} ${dir}`, false, {
    env: {
  ...
  const answers = await inquirer.prompt([{
    ...
    type: 'input',
    name: 'git_url',
    message: 'Enter a git repository url\n(If you don\'t have one, create one now using either Github or Bitbucket,\nor run `boostid create-github-repo <username> <reponame>`)',
    validate: (git_url) => {
      try {
        ci.setGitUrl(git_url);
        return true;
    ...
    // choices: () => {
    //   const userObj = config.get('github') || {};
    //   const users = Object.keys(userObj);
    //   return [
    //     ...users.map((value) => ({ name: `Username: ${value}`, value })),
    ...
    //   ];
    // },
    default: argv.git_url,
    // default: () => argv.git || `https://github.com/${ci.getOwner()}/${sitename}`,
  }, {
    type: 'password',
    name: 'ciToken',
    message: 'CircleCI user API token (obtain from https://circleci.com/account/api)',
    mask: '*',
    when: () => !config.get(`git.${ci.getOwner()}.ciToken`),
    validate: (_) => !!_,
  }, {
    type: 'input',
    name: 'sshKeyPath',
  ...
  const { git_url, ciToken, testsDir, slackWebhook, ciConfig,
  ...
  if (ciToken)
    config.setGlobal(`git.${ci.getOwner()}.ciToken`, ciToken);

  if (!fs.existsSync(path.resolve(dir, 'boostid.config.js'))) {
...
`;
  ...
  await run(`git remote set-url --add origin ${git_url}`, false, { cwd: dir });
  ...
  await followCI({ git_url, slackWebhook });
