...
exports.init = (argv) => {
  ...
  if (argv.machineToken) config.machineToken = argv.machineToken;
    ...
    try {
      ...
      if (token.token) {
        configStore.set('machineToken', token.token);
        config.machineToken = token.token;
      }
    } catch (_) {}
  }
  ...
  else if (process.env.PANTHEON_SITE_ID) config.site = process.env.PANTHEON_SITE_ID;
...
exports.setGlobal = (key, val) => {
  configStore.set(key, val);
  config[key] = val;
};

exports.setLocal = (key, val) => { throw 'Not yet implemented'; };
