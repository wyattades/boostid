...
module.exports = async (argv) => {
  ...
  if (argv.results === 'S3') {
    ...
    envs.PANTHEON_SITE_NAME = config.get('site');
  ...
  if (argv.devBoostid) {
    logger.info('Copying local boostid directory');
    argv.devBoostid = argv.devBoostid.replace(/\/*$/, '/');
    await run(`rsync -av ${argv.devBoostid} ./tmp_boostid --exclude node_modules --exclude .git --exclude wades`, 'ignore');
  }
...
CMD ["./node_modules/boostid/scripts/run-tests.js", "local"]
`;
  ...
  try { await run(`docker container rm ${containerId}`, 'ignore'); } catch (_) {}
