...
module.exports = async (argv) => {
  ...
  if (argv.devBoostid) {
    ...
    await run(`rsync -av ${argv.devBoostid} ./tmp --exclude node_modules --exclude .git`, true);
  }
...
`;
  ...
  const containerId = (await run(`docker ps -lq`)).trim();

  if (failed) {
    console.log('ccc', containerId);
    logger.info('Copying snapshot files to ./__image_snapshots__');
