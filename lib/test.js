...
const localTests = async (argv) => {
  ...
  if (!fs.existsSync('node_modules/puppeteer/.local-chromium'))
    log.warn('It doesn\'t look like `puppeteer` has a local chromium installation, which may cause issues.\n  Suggest to run: npm install --save puppeteer@1.11.0\n');
...
exports.coverage = async (argv) => {
  ...
  const passed = (argv.docker === false || argv.dev)
